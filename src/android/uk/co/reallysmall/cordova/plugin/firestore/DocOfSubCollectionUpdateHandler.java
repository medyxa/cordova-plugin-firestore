...
public class DocOfSubCollectionUpdateHandler implements ActionHandler {
    ...
    public boolean handle(JSONArray args, final CallbackContext callbackContext) {
        try {
            ...
            FirestoreLog.d(FirestorePlugin.TAG, "Updating document of sub collection");
            ...
            try {
                firestorePlugin.getDatabase().collection(collection).document(docId).collection(subCollection).document(docOfSubCollectionId).update(JSONHelper.toSettableMap(data)).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        ...
                        FirestoreLog.d(FirestorePlugin.TAG, "Successfully updated document of sub collection");
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        callbackContext.error(e.getMessage());
                        FirestoreLog.w(FirestorePlugin.TAG, "Error updating document of sub collection", e);
                    }
                });
            } catch (Exception e) {
                FirestoreLog.e(FirestorePlugin.TAG, "Error processing document of sub collection update in thread", e);
                callbackContext.error(e.getMessage());
            }
            ;
        } catch (JSONException e) {
            FirestoreLog.e(FirestorePlugin.TAG, "Error processing document of sub collection update", e);
            callbackContext.error(e.getMessage());
        }
